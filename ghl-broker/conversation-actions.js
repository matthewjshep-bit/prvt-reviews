...
const EXECUTORS = {
  async add_tags({ client, contactId, action, store, locationId, draft, deps }) {
    ...
    if (typeof deps?.onTagsChanged === "function") await deps.onTagsChanged({ contactId, tags: action.tags, party: draft?.party || null, change: "added" }).catch(() => {});
  ...
  async remove_tags({ client, contactId, action, store, locationId, draft, deps }) {
    ...
    if (typeof deps?.onTagsChanged === "function") await deps.onTagsChanged({ contactId, tags: action.tags, party: draft?.party || null, change: "removed" }).catch(() => {});
